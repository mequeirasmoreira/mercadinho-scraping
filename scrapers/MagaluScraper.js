...
/**
 * Scraper para o site do Magalu (categoria de supermercado)
 */
class MagaluScraper extends SupermarketScraper {
  constructor() {
    super('Magalu');
    this.baseUrl = 'https://www.magazineluiza.com.br';
    this.categories = [{
        name: 'Leite',
        url: '/supermercado/mercearia/leites/'
      },
      {
        name: 'Arroz',
        url: '/supermercado/alimentos-basicos/arroz/'
      },
      {
        name: 'Feijão',
        url: '/supermercado/alimentos-basicos/feijao/'
      }
    ];
  }
  ...
  async scrape() {
    ...
    try {
      for (const category of this.categories) {
        ...
        // Seletor para os itens de produto no Magalu
        $('[data-testid="product-card"]').each((index, element) => {
          try {
            const titleElement = $(element).find('[data-testid="product-title"]');
